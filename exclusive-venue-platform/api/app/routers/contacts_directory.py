...
def _deal_stats(client: Client, contact_ids: list[str]) -> DealStats:
    ...
    try:
        proposals = (
            client.table("proposals").select("id, enquiry_id, status, currency").in_("enquiry_id", enquiry_ids).execute()
        )
    except APIError as exc:
        _raise_for_postgrest(exc)
    ...
    if signed_proposal_ids:
        try:
            venues = client.table("proposal_venues").select("quote_total").in_("proposal_id", signed_proposal_ids).execute()
        except APIError as exc:
            _raise_for_postgrest(exc)
...
TimelineType = Literal[
    "onboarded", "enquiry_received", "brief_parsed", "proposal_sent", "proposal_won", "proposal_declined", "activity"
]
...
def _brief_fields(brief: dict[str, Any]) -> dict[str, str]:
    ...
    if brief.get("duration_hours"):
        fields["Duration"] = f"{brief['duration_hours']}h" + (f" · {brief['time_of_day']}" if brief.get("time_of_day") else "")
    ...
    if brief.get("budget_status") == "tbc":
        ...
        fields["Budget"] = f"{int(brief['budget_amount']):,}" + (" per head" if brief.get("budget_basis") == "per_head" else "")
    return fields
...
def _organisation_timeline(client: Client, org: dict, contacts: list[dict]) -> list[TimelineEntry]:
    ...
    for e in enquiries.data:
        ...
        entries.append(
            TimelineEntry(
                ...
                title=(e["briefs"] and max(e["briefs"], key=lambda b: b["version"])["event_type"]) or "New enquiry",
    ...
    if enquiry_ids:
        ...
        for p in proposal_rows:
            ...
            venue_names = [venue_name_by_id[pv["venue_id"]] for pv in pv_list if pv["venue_id"] in venue_name_by_id]
